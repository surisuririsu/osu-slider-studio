...
export default class Slider {
  ...
  getNearPoint(point) {
    const nearPoints = this.segments.map((segment) =>
      segment.getNearPoint(point)
    )
    const nearestPoint = nearPoints.reduce(
      (acc, curr, index) => {
  ...
  getNearEdge(point) {
    ...
    const nearestEdge = nearEdges.reduce(
      (acc, curr, index) => {
  ...
  draw(ctx, tickDist) {
    const fullDist = this.segments.reduce(
      (acc, curr) => acc + curr.getDist(),
      0
    )
  ...
  getMidpoint() {
    const boundaries = this.segments.reduce(
      (acc, curr) => {
  ...
  getFullDist() {
    return this.segments.reduce((acc, curr) => acc + curr.getDist(), 0)
  }
  ...
  getOsuCode(tickDist) {
    ...
    if (this.getLength() === 1 && sliderType !== 'spline') {
      ...
      sliderType = 'bezier'
      allPoints = this.segments.reduce(
        (acc, curr) => acc.concat(curr.getBezierPoints()),
        []
      )
    }
